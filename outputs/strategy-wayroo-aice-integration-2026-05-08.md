...
**Status (2026-05-23):** v4.3 — Motion 3 two-option pricing framing reflected in open questions (Option 1 bundle vs. Option 2 rev-share on Aice license revenue, per Expansion v3.3). Headless back office framing as Power 4 + Pillar 1 reframe. Motion 5 replaced (Pillars-bundle → Wayroo + Aice Back Office Lite, headless via Aice dev platform). New Motion 8 (Developer Platform access for existing ByDesign clients) and Motion 9 (Aice-pitched Wayroo cart inside existing Aice DSO clients). AI Report Builder retired from 2026 BD roadmap. AiCE payments rev-share hard-capped at 25%.

**Author:** Sam Atieh · **Version:** v4.3 · **Audience:** Leadership, Product, GTM, BD
**Companion doc:** Expansion Strategy — AiCE Channel v3.3 (revenue mechanics, weekly tactical action list)

> **Document set — AiCE Partnership**
>
> * **Thesis / governance:** [Retail Success x AiCE Partnership (Integration Strategy v4.3)](https://bydesign.atlassian.net/wiki/spaces/WAYR/pages/444170241) — WAYR space (this page)
> * **Revenue mechanics:** [Expansion Strategy — AiCE Channel v3.3](https://bydesign.atlassian.net/wiki/spaces/WAYR/pages/443809798) — WAYR space
...
**Source file (team-os GitHub workspace):** `team-os/product-development/product/strategy/strategy-wayroo-aice-integration-2026-05-08.md` v4.3.
...
## AiCE Thesis PDF Reconciliation (May 18) + Motion 6 (May 21) + Headless Reframe (May 23)
...
### Surface areas (aligned with Expansion Strategy v3.3 motion numbering)
...
| 4 | Wayroo → BD-base cross-sell | Motion 3 — Wayroo + AiCE pitched to existing BYD clients. **Two pricing paths** (Option 1 bundle when Wayroo value prop resonates; Option 2 rev-share on Aice license revenue when it doesn't — typically Non-C&C ICP 1c accounts). Retention play + Wayroo cross-sell. |
...
| **Motion 3 Option 1 bundle conversions vs. Option 2 rev-share conversions** | **Track per-account; pilot first 3 in Q3 2026** |
...
**What:** A single SKU sold by both sales teams: "Wayroo + AiCE." One contract, one combined price, defined rev-share split, joint implementation, joint support tier-1 routing. **Co-roadmap** reviewed quarterly so both sides have visibility on adjacent surface areas and we can sequence shared bets together. **Motion 3 has a two-option commercial fork: Option 1 (full bundle pricing when Wayroo value prop fits) and Option 2 (rev-share on Aice license revenue when Wayroo isn't a fit — typically Non-C&C BYD accounts).**
...
**Key initiatives:** Term sheets for Motions 1 and 2 signed (June 2026 — **Motion 6 and Motion 9 mechanics baked into Motion 2 term sheet as payments rev-share extensions; Motion 3 Option 2 license-rev-share baked in as a Motion 3 term-sheet clause**) · Bundle pricing finalized (July 2026) · **Motion 5 Back Office Lite + Motion 8 Developer Platform commercial structures drafted (July 2026)** · Master agreement template + deal-reg process (August 2026) · Co-roadmap cadence established (August 2026) · First joint deal closed (Q4 2026).
...
* **Leading Motion 3 pitches with the rev-share fallback (Option 2).** Option 1 bundle is the default; Option 2 only after discovery shows Wayroo isn't a fit.

## 6. Open Questions (updated May 23)
...
6. **Bundle pricing structure** — per-rep, per-DSO, or hybrid? Morgan to model. Applies to Motion 3 Option 1 bundle component.
...
11. **Motion 3 — two-option pricing path resolution.** Which existing ByDesign clients are highest-fit for **Option 1 bundle pricing** (Wayroo value prop resonates — typically C&C / Hybrid ICP 1a/1b)? Which are likely **Option 2 rev-share** (Non-C&C ICP 1c — Aice fits, Wayroo doesn't)? Sub-questions: (a) sequencing of the first 3 pilots; (b) under Option 1, who pays for the AiCE component (rep PLG / corporate-paid / hybrid); (c) under Option 2, what % of Aice's license revenue does Retail Success take — pricing mood board Structure C hypothesis: open at 20%, range 15–25%; (d) deal-reg clarity when the BYD relationship pre-dates Aice involvement.
...
15. **Motion 5 commercial structure** — what does the headless Back Office Lite subscription price at? Rev-share with Aice for the dev platform UI layer? Lite-to-full BD upgrade path / cannibalization mitigation?
16. **Motion 8 Developer Platform pricing** — monthly subscription per BYD client? Per developer seat? How is Aice paid (flat platform fee, % of subscription, transaction-style)?
17. **Aice developer platform code ownership** — does Aice own the platform code, or is it a joint product? Termination / data-portability implications for both Motion 5 startups and Motion 8 existing BYD clients.
18. **AI Report Builder retirement communication** — which existing BYD accounts were waiting on it? Communication plan + dev platform onboarding for those accounts.
19. **Pillars status** — Motion 5 replaced with headless lite; should Pillars's ~10 clients become Motion 1 referrals, or is the Pillars angle deprioritized entirely?
20. **Motion 9 sequencing** — when does Aice start pitching Wayroo cart to its existing DSO clients? Pre- or post-Motion 1/2 term sheet signing? Co-pitch vs. Aice-only?
21. **(NEW) Aice's appetite for sharing license revenue under Motion 3 Option 2** — is Aice willing to share % of its license revenue from Non-C&C BYD accounts where Wayroo isn't licensed directly? What % feels reasonable from their side? Required before Motion 3 term-sheet drafting.

## References

* Companion: Expansion Strategy — AiCE Channel v3.3 (this space) — revenue mechanics, weekly tactical action list, full Motion 3 (two-option pricing) / 6 / 8 / 9 writeups + Motion 5 (headless lite).
* Partnership pricing mood board (Phase 5): GTM space — Structures A–H including Motion 3 two-option fork, Motion 5/6/8/9 mechanics.
* Unified 2026 strategy: PM-OS/context-library/strategy/ByDesign-Wayroo-Unified-Strategy-2026.md (Pillar 3, Section 5).

_Last updated: 2026-05-23 (v4.3 — Motion 3 two-option pricing framing reflected: OQ #11 reframed; surface area #4 + Pillar 2 + "What we are NOT doing" updated to reference Option 1 bundle vs. Option 2 rev-share on Aice license revenue (per Expansion v3.3 + Mood Board Structure C). v4.2 — Headless back office framing added as Power 4 + Pillar 1 reframe; Motion 5 replaced; new Motion 8 + Motion 9; AI Report Builder retired; Motion 1 metric — Aice closes 1-2 DSOs/month. AiCE payments rev-share hard cap at 25% retained). Feedback requested from: Morgan (pricing for Motions 3 / 5 / 8 / 9 + API terms + Motion 6 split), Josh (integration architecture + dev platform scope + Motion 4 / 7 onboarding + Motion 6 processor-swap feasibility), Legal (term sheet, change-of-control, co-roadmap, dev platform code ownership), Gordon (Motion 1.5 joint marketing + co-launch event + AI Report Builder retirement communication). Next review: end of June 2026._
